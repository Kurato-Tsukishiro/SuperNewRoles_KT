...
| モード名     |                                        簡易説明                                        |
| ------------ | :------------------------------------------------------------------------------------: |
| [仕事人モード](#仕事人モード) | タスクの速度を競うモードです。一人プレイでタスクの練習する為のモードとしても遊べます。 |
...
### 説明
- オンラインモードでタスクの速度を競うモードです。
  - インポスターは配役されず全員クルーです。勝者は一人のみです。
- ローカルでの一人プレイでタスクを練習する為の使用もお勧めです。

<br>

- サボタージュや緊急招集が設定秒数の範囲で自動的に行う設定も存在します。
  - コミュニティサボタージュを直すか、直さないか等の戦略性が楽しめます。
  - 試合に近い形で練習ができます。

### モード設定
|名前|説明|
| :-- | :-- |
| 仕事人モードの設定|仕事人モードのカスタム設定を有効にするかの設定です。|
| 緊急招集が自動で行われるか | 緊急招集が自動で行われるかの設定です。 |
| [個別設定] 自動緊急招集が起きる時間 | 自動緊急招集が起きる時間のカスタム設定を有効にするかの設定です。<br>この設定をオフにした場合、SNR側の推奨設定が適応されます。 |
| 緊急招集が発動する時間 | 緊急招集が発動する時間の中央値を設定します。 |
| 緊急招集が発動する時間の誤差 | 緊急招集が発動する時間の誤差の絶対値を設定します。<br>「[緊急招集が発動する時間] ± [此処で設定した値]」sの範囲で<br>緊急招集が起きます。 |
| タスクが終わらなかった場合敗北するか | 指定ターン数までにタスクを終わらせられた人がいない場合全員が敗北するかの設定です<br>(基本的には一人プレイ想定の設定です) |
| 強制敗北するターン数 |  強制敗北する指定ターン数の設定です。 |
| サボタージュを発動させるか | 試合中にサボタージュを発生させるかの設定です。 |
| [個別設定] サボタージュ | サボタージュに関するカスタム設定を有効にするかの設定です。<br>この設定をオフにした場合、SNR側の推奨設定が適応されます。 |
| サボタージュを自動で直すか | サボタージュが自動敵に直るかの設定できます。<br>この設定が有効であった場合も、手動で直すことは可能です。 |
| サボタージュが直る時間 | サボタージュが直る時間の中央値を設定します。 |
| サボタージュが直る時間の誤差 | サボタージュが直る時間の誤差の絶対値を設定します。<br>「[サボタージュが直る時間] ± [此処で設定した値]」sの範囲で<br> サボタージュが直ります。 |
| ドアが閉まるまでの時間 | ターン開始からドア閉めは何秒後に起こるか設定します。 |
| ドア閉めの間隔 | 初回ドア閉め後何秒間隔でドアを閉めるか設定します |
| 停電が入るターン数の上限 | 試合開始から何ターンまで停電のみを行うかの設定です。<br>ここで設定したターン数経過以降コミュニティサボタージュや衝突回避サボタージュ等停電以外のサボタージュが発動するようになります。 |
| 停電が入る秒数 | ターン開始から初回停電が入るまでの時間の設定です。 |
| 停電以外のサボが入るまでの秒数 | ターン開始から停電以外のサボタージュが入るまでの時間の設定です。 |
| サボタージュが発動する間隔 | サボタージュが発動する間隔の中央値を設定します。 |
| サボタージュが発動する間隔の誤差 | サボタージュが発動する間隔の誤差の誤差の絶対値を設定します。<br>「[サボタージュが発動する間隔] ± [此処で設定した値]」sの範囲で<br> サボタージュが発動します。 |